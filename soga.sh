...
#        return 0
#    fi
    bash <(curl -Ls https://gitlab.com/xiaozhan-sb/azp/-/raw/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/soga -N --no-check-certificate https://gitlab.com/xiaozhan-sb/azp/-/raw/main/soga.sh
    if [[ $? != 0 ]]; then
        echo ""
