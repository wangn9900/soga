...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://gitlab.com/xiaozhan-sb/azp/-/raw/main/latest.json" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://gitlab.com/xiaozhan-sb/azp/-/raw/main/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://gitlab.com/xiaozhan-sb/azp/-/raw/main/soga-cracked-linux64.tar.gz"
    ...
    curl -o /usr/bin/soga -Ls https://gitlab.com/xiaozhan-sb/azp/-/raw/main/soga.sh
