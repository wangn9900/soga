...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://raw.githubusercontent.com/wangn9900/soga/main/latest.json" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://github.com/wangn9900/soga/releases/download/v1.0.1/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载 soga 失败，请确保你的服务器能够下载 CloudFlare 的文件${plain}"
            exit 1
        fi
    else
        last_version=$1
        url="https://github.com/wangn9900/soga/releases/download/v1.0.1/soga-cracked-linux64.tar.gz"
    ...
    curl -o /usr/bin/soga -Ls https://raw.githubusercontent.com/wangn9900/soga/main/soga.sh
